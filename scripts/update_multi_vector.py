...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description='Update benchmark for ColBERT-style multi vectors')
    ...
    parser.add_argument('--data-dir', default='data/update_test_multi_vector')
    parser.add_argument('--generation-batch-size', type=int, default=2048)
    parser.add_argument('--vector-chunk-rows', type=int, default=4)
...
def generate_shards(args: argparse.Namespace) -> None:
    ready_paths = validate_ready_shards(
        ...
        kind='multi_vector',
        attrs={'vector_dim': args.vector_dim, 'token_count': args.token_count, 'common_data_version': COMMON_DATA_VERSION},
        required_datasets=('id', 'uuid1', 'uuid2', 'platform', 'text', 'vector'),
    )
    attrs = {
        'kind': 'multi_vector_update',
        'rows': 0,
    ...
    for shard_idx, path in enumerate(paths):
        ...
        write_mixed_update_shard(
            ...
            rng=rng,
            attrs=shard_attrs,
            vector_shape_tail=(args.token_count, args.vector_dim),
            vector_chunks=(min(max(1, args.vector_chunk_rows), rows), args.token_count, args.vector_dim),
        )
...
def update_records(args: argparse.Namespace) -> None:
    ...
    if args.executor_kind == 'process':
        ...
        result = run_update_operations(
            ...
            description='update multi',
            prefetch_batches=prefetch_batches,
            executor_kind='process',
            executor_initializer=init_update_process_worker,
            executor_initargs=(config, args.collection_name, args.timeout, args.multi_vector_mode, args.consistency_level),
        )
    else:
